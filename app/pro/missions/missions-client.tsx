...
import { dismissCancelledSlot } from "./missions.actions";
    ...
    <div className="space-y-8 mt-6">
      {pending.length > 0 && (
        <section>
          ...
          <div className="border rounded-lg divide-y">
            {pending.map((slot) => (
              <MissionRow key={slot.slotId} slot={slot} />
            ))}
          </div>
        </section>
      )}
      ...
      {cancelled.length > 0 && (
        <section>
          ...
          <div className="border rounded-lg divide-y">
            {cancelled.map((slot) => (
              <MissionRow key={slot.slotId} slot={slot} showDismiss />
            ))}
          </div>
        </section>
      )}
...
interface MissionRowProps {
  slot: ProviderMissionRow;
  showDismiss?: boolean;
}

function MissionRow({ slot, showDismiss = false }: MissionRowProps) {
